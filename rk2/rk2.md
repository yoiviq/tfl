...
Докажем нерегулярность по лемме о накачке: длина накачки $n$, $w = (ab)^naab^n$. Так как $|xy| \le n$, то $xy$ содержит только пары $ab$. Возьмем $i = 0$, тогда из $w_1$ ушла как минимум одна буква $a$, значит в $w_2$ должно быть на как минимум одну $b$ меньше. Но переорганизовать разбиение $w' = w_1a^nw_2$ таким образом, чтобы в $w_2$ стало меньше $b$, мы не можем так как чтобы уменьшить кол-во $b$ в $w_2$ нужно, чтобы $aa$ принадлежало $w_1$, что запрещено.

Следовательно, язык не регулярный. 
