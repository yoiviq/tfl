...
##### Рассмотрим отображение $F_1: \Sigma^* \to \mathbb{N}$ $$F_1(w) := |w|_p$$ 
...
##### Рассмотрим отображение $F_2: \Sigma^* \to \mathbb{N}$ $$F_2(w) := |w|_{bp}$$ 
...
Определим функцию $f: \Sigma \to \mathbb{N}$ и пусть $$f(a) := 1, f(b) := 0, f(p) := 0$$ 

##### Рассмотрим отображение  $F_3: \Sigma^* \to \mathbb{N}$ $$F_3(w) := \sum_{i=0}^{|w|-1} f(w[i])2^{|w|-i-1}$$ 
...
##### Таким образом, определяем над множеством $\Sigma^*$ лексикографический порядок через отображение $F: \Sigma^* \to \mathbb{N}^3$ $$F(w) := (F_1, F_2, F_3)$$ 
